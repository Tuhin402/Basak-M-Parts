...
<body class="preload-wrapper">
    ...
    <div id="wrapper">
        ...
        <!-- page-title -->
        <?php 
            if ($cat_id && $company_id) {
                $catcompQuery = "SELECT pc.category AS category_name, co.name AS company_name FROM category_company cc
                        JOIN product_category pc ON cc.cat_id = pc.id JOIN company co ON cc.company_id = co.id WHERE cc.cat_id = $cat_id AND cc.company_id = $company_id";
            
                $catcomp = $obj->fetch($catcompQuery);
            
                if (!empty($catcomp)) {
                    $category_name = $catcomp[0]['category_name'];
                    $company_name = $catcomp[0]['company_name'];
                }
            } elseif ($cat_id) {
                $catcompQuery = "SELECT category FROM product_category WHERE id = $cat_id";
                $catcomp = $obj->fetch($catcompQuery);
                if (!empty($catcomp)) {
                    $category_name = $catcomp[0]['category'];
                }
            } elseif ($company_id) {
                $catcompQuery = "SELECT name FROM company WHERE id = $company_id";
                $catcomp = $obj->fetch($catcompQuery);
                if (!empty($catcomp)) {
                    $company_name = $catcomp[0]['name'];
                }
            }
        ?>
        <?php 
        ...
        <div class="page-title" style="background-image: url('../Admin/uploads/banner/<?= htmlspecialchars($ban['b2b_image']) ?>');">
            <div class="container">
                ...
                <ul class="breadcrumbs d-flex align-items-center justify-content-center">
                    ...
                    <li><a href="products-grid.php?cat_id=<?= $cat_id ?>&company_id=<?= $company_id ?>" class="text text-caption-1"><?= htmlspecialchars($category_name) ?></a></li>
                    <li><i class="icon icon-arrRight"></i></li>
                    <li><a href="products-grid.php?cat_id=<?= $cat_id ?>&company_id=<?= $company_id ?>" class="text text-caption-1"><?= htmlspecialchars($company_name) ?></a></li>
                    <li><i class="icon icon-arrRight"></i></li>   
                    <li>Products</li>
                </ul>
            </div>
            ...
            <div class="container">
                ...
                <div class="wrapper-control-shop">
                    <div class="tf-grid-layout wrapper-shop tf-col-4" id="gridLayout">
                        <?php 
                        if ($cat_id && $company_id) {
                            $query = "SELECT * FROM products WHERE cat_id = $cat_id AND company_id = $company_id ORDER BY sort_order ASC";
                            $products = $obj->fetch($query);
                        } else {
                            echo "Invalid category or company!";
                            exit;
                        }
