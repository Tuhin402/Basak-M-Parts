...
if (count($products) > 0) {
    foreach ($products as $val) {
        ...
        $categoryData = $obj->fetch("SELECT category FROM product_category WHERE id = $commonCatId");
        $cat = isset($categoryData[0]['category']) ? $categoryData[0]['category'] : 'Unknown';

        $companyData = $obj->fetch("SELECT name FROM company WHERE id = $commonCompId");
        $comp = isset($companyData[0]['name']) ? $companyData[0]['name'] : 'Unknown';

        $sqlRating = "SELECT AVG(rating) AS avg_rating FROM product_reviews WHERE product_id = $product_id";
        $ratingData = $obj->fetch($sqlRating);
        $avg_rating = isset($ratingData[0]['avg_rating']) ? round($ratingData[0]['avg_rating'], 1) : 0;
        ?>
        <tr>
            <td>
            ...
            <td><p class="text-muted"><?php echo $cat ?></p></td>
            <td><p class="text-muted"><?php echo $comp ?></p></td>
            <td>
                <span class="badge p-1 bg-light text-dark fs-12 me-1">
                    <i class="bx bxs-star align-text-top fs-14 text-warning me-1"></i> <?= $avg_rating ?>
                </span>
            </td>
            <td>
