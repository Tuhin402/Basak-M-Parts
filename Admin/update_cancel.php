...
if ($action === 'approve') {
    $newStatus = "Cancellation Approved";
} elseif ($action === 'reject') {
    $newStatus = "Cancellation Rejected";
} else {
...
$sql_cancel = "UPDATE order_cancel SET admin_notice = ?, admin_proof = ?, order_status = ? WHERE id = ? AND order_id = ?";
...
$sql_orders = "UPDATE orders SET order_status = ? WHERE order_id = ?";
...
$sql_email = "SELECT banking_name, user_email FROM order_cancel WHERE id = ?";
...
if (!empty($customerEmail)) {
    ...
    <html>
    <head>
      <title>Order Cancellation Update</title>
    </head>
    <body>
      <p>Dear {$customerName},</p>
      <p>Your order cancellation has been processed successfully. Please find the details below:</p>
      <p>{$admin_notice}</p>
...
echo json_encode(["success" => true, "message" => $message, "shiprocket_response" => $shiprocket_response_raw]);
exit;
?>