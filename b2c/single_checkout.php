    ...
    <div id="wrapper">
        ...
        <section class="flat-spacing">
            <div class="container">
                <div class="row">
                    <?php 
                    $shipQuery = "SELECT b2c_special_discount, shipping_price, mrp, b2c_discount FROM products WHERE id = '$product_id'";
                    $shippingCost = $obj->fetch($shipQuery);
                    $shipping_price = $shippingCost[0]['shipping_price'];
                    $b2c_discount = $shippingCost[0]['b2c_discount']; 
                    $mrp = $shippingCost[0]['mrp']; 
                    $b2c_special_discount = $shippingCost[0]['b2c_special_discount']; 
                    $discounted_amount = $total_amount - ($total_amount * ($b2c_special_discount / 100)); 
                    $discount_amount = $total_amount * ($b2c_special_discount / 100); 
                    
                    $total_price = $discounted_amount + $shipping_price;
                    $round_total_price = round($total_price);
                    $total_price_cod = $total_amount + $shipping_price;
                            ...
                            <div class="sidebar-checkout-content">
                                ...
                                <div class="list-product">
                                    <div class="item-product">
                                        <a href="product-detail.php?product_id=<?= base64_encode($product_id) ?>" class="img-product" style="width: 100px;">
                                            <img src="../Admin/uploads/products/<?= htmlspecialchars($product_image) ?>" alt="<?= htmlspecialchars($product_name) ?>" style="width: 100%; height: 100%; object-fit: cover">
                                        </a>
    ...
    <script>
        ...
        document.getElementById("cod_order").addEventListener("click", function(e) {
            ...
            const formData = {
                ...
                price: "<?= $product_price ?>",
                shipping_price: "<?= $shipping_price ?>",
                total_price: "<?= $round_total_price_cod ?>",
                hsn: "<?= htmlspecialchars($hsn) ?>",
                sku: "<?= htmlspecialchars($sku) ?>"
            };
