...
$user_landmark = $_SESSION["user_landmark"];
    ...
    <div id="wrapper">
        ...
        <section class="flat-spacing">
            <div class="container">
                <div class="row">
                    <?php 
                    $productQuery = "SELECT c.pro_id, c.qty, c.total_price, c.shipping_price, p.name, p.image_1, p.sku, p.part, p.b2c_price, p.b2c_special_discount, p.b2c_discount, p.mrp FROM cart c INNER JOIN products p ON c.pro_id = p.id WHERE c.b2c_id = '$userId'";
                    ...
                    foreach ($cartItems as $item) {
                        ...
                        $cod_price = $item['b2c_price'] * $item['qty'];  
                        $total_b2c_price += $cod_price;
                        $shipping_amount += $item['shipping_price'];
                    }
                    
                    $total_price = $total_amount + $shipping_amount; 
                    $round_total_price = round($total_price);
                    $total_price_cod = $total_b2c_price + $shipping_amount;  
                    $round_total_price_cod = round($total_price_cod);
                    ?>
                    <div class="col-xl-5">
                            ...
                            <div class="sidebar-checkout-content">
                                ...
                                <div class="list-product">
                                    <?php if (!empty($cartItems)) { 
                                        foreach ($cartItems as $item) { 
                                            $b2c_special_discount = $item['b2c_special_discount'];
                                            $discounted_amount = $item['b2c_price'] - ($item['b2c_price'] * ($b2c_special_discount/100));
                                            $discount_amount = $item['b2c_price'] * ($b2c_special_discount/100); ?>
                                            <div class="item-product">
                                                ...
                                                <div class="content-box d-flex flex-column align-items-start">
                                                    ...
                                                    <div class="tf-product-info-price">
                                                        <p class="text-button price-on-sale font-2"><span><?= $item['qty'] ?></span> X <span class="price">₹ <?= number_format($item['b2c_price'], 2) ?></span></span></p>
                                                        <div class="compare-at-price font-2"><?= htmlspecialchars($item['mrp']) ?></div>
                                                        <?php if($item['b2c_discount'] != 0) { ?>
                                                        <div class="badges-on-sale text-btn-uppercase">-<?= htmlspecialchars($item['b2c_discount']) ?>%</div>
                                                        <?php } ?>
                                                    </div>
                                                    <p class="badge bg-info text-dark"><i class="bi bi-percent me-1"></i>All taxes included</p>
                                                    <?php if($b2c_special_discount != 0) { ?>
                                                    <div class="info">
                                                        <p class="text-primary" style="font-size:14px;"><i class="bi bi-tags me-1"></i>Additional <strong><?= number_format($b2c_special_discount, 2) ?>%</strong> off on online payment - <strong>₹<?= number_format($discounted_amount, 2) ?></strong></p>
                                                    </div>
                                                    <?php } ?>
                                                </div>
                                            </div>
                                ...
                                <div class="sec-total-price">
                                    <div class="bottom">
                                        ...
                                        <h5 class="d-flex justify-content-between mb-5">
                                            ...
                                            <span class="total-price-checkout">- ₹<?= number_format($discount_amount, 2) ?></span>
                                        </h5>
                                        ...
                                        <h5 class="d-flex justify-content-between">
                                            <span>Total(COD)</span>
                                            <span class="total-price-checkout">₹<?= number_format($round_total_price_cod, 2) ?></span>
                                        </h5>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        document.getElementById("cod_order").addEventListener("click", function(e) {
            ...
            const formData = {
                ...
                landmark: "<?= htmlspecialchars($user_landmark) ?>",
                pin: "<?= htmlspecialchars($user_pin) ?>",
                total_amount: "<?= $round_total_price_cod ?>",
                products: [
                    <?php foreach ($cartItems as $item) { ?>
                    {
                        product_id: "<?= $item['pro_id'] ?>",
                        ...
                        quantity: "<?= $item['qty'] ?>",
                        price: "<?= $item['b2c_price'] ?>",
                        total_price: "<?= $item['b2c_price'] * $item['qty'] ?>",
                        hsn: "<?= htmlspecialchars($item['sku']) ?>",
                        sku: "<?= htmlspecialchars($item['part']) ?>"
                    },
                    <?php } ?>
